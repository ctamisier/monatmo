import type { HomesDataResponse, HomeStatusResponse, RoomMeasureResponse, SetStateResponse, PublicDataResponse } from '../types/netatmo'
import { getValidToken, getRefreshedToken } from './auth.service'
import { i18n } from '../i18n'
import { PUBLIC_DATA_COORDINATE_DELTA } from '../constants/chart'
...
async function handleErrorResponse(response: Response): Promise<never> {
  ...
  throw new Error(i18n.global.t('errors.errorStatus', {
    status: response.status,
    error: error.error?.message || i18n.global.t('netatmo.unknownError'),
  }))
}
...
export async function setModuleState(
  ...
  return response.json() as Promise<SetStateResponse>
}
...
export async function fetchPublicData(
  ...
  return apiFetch<PublicDataResponse>('getpublicdata', {
    lat_ne: String(lat + PUBLIC_DATA_COORDINATE_DELTA),
    lon_ne: String(lon + PUBLIC_DATA_COORDINATE_DELTA),
    lat_sw: String(lat - PUBLIC_DATA_COORDINATE_DELTA),
    lon_sw: String(lon - PUBLIC_DATA_COORDINATE_DELTA),
    'filter': 'true',
  })
}
